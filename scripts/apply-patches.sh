...
# configuration gets resolved early in build.gradle and barfs if we touch it.
{
    ...
    echo "afterEvaluate {"
    echo "    def webmcHasLwjglStubs = file('../addons/lwjgl-stubs/src/main/java').exists()"
    ...
    echo "        if (c != null) {"
    echo "            if (webmcHasLwjglStubs) { c.exclude group: 'org.lwjgl' }"
    ...
    echo "    implementation 'org.teavm:teavm-jso-apis:0.13.1'"
